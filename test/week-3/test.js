...
test("html to segments", async (t) => {
  const html = await fs.readFile(
    path.join(__dirname, "fixtures", "012_pff_early.html"),
    "utf-8"
  )
  const segments = await htmlToSegments({
    html,
    first_para: "Every team that has won",
    second_para: "We’ll again do three picks",
    voices,
    sound_effects_dir:
  ...
  let solution = await fs.readFile(
    path.join(__dirname, "fixtures", "013_segments.json"),
    "utf-8"
  )
