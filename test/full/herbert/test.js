...
test("html to segments", async (t) => {
  const html = await fs.readFile(
    path.join(__dirname, "fixtures", "014_pff_herbert.html"),
    "utf-8"
  )
  const segments = await htmlToSegments({
    html,
    first_para: "ensured there was no doubt",
    second_para: "no quarterback competition this week",
    voices,
    sound_effects_dir:
  ...
  let solution = await fs.readFile(
    path.join(__dirname, "fixtures", "015_segments.json"),
    "utf-8"
  )
